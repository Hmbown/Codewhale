...
for tape in tape.tsv tapes/edge-cases.tsv; do
    ...
    for mode in animated reduced-motion; do
        ...
        java -cp "$kotlin_classpath" codewhale.pet.ConformanceKt whale-points.tsv "$@" < "$tape" > "conformance-results/$expression-$name-$mode-kotlin.txt"
        diff -u "conformance-results/$expression-$name-$mode-ts.txt" "conformance-results/$expression-$name-$mode-kotlin.txt"
        printf 'PASS Kotlin %s %s %s\n' "$expression" "$name" "$mode"
    done
done
done
