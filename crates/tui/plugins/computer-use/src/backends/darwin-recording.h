...
static void cuWait(dispatch_semaphore_t sem, NSTimeInterval seconds) {
...
static id cuRecord(NSDictionary *args) {
  ...
  [stream startCaptureWithCompletionHandler:^(NSError *e){error=e;dispatch_semaphore_signal(sem);}];cuWait(sem,15);
  if(error) @throw [NSException exceptionWithName:@"recording" reason:error.localizedDescription userInfo:nil];
  ...
  while(!cuStopRecording && (!deadline || deadline.timeIntervalSinceNow>0))
    [[NSRunLoop currentRunLoop] runUntilDate:[NSDate dateWithTimeIntervalSinceNow:0.05]];
  [stream stopCaptureWithCompletionHandler:^(NSError *e){error=e;dispatch_semaphore_signal(sem);}];cuWait(sem,15);
  ...
  [rec.writer finishWritingWithCompletionHandler:^{dispatch_semaphore_signal(sem);}];cuWait(sem,15);
