...
''')
    ...
    def test_dsh_bundle_packages_are_imported_natively_not_converted_here(self):
        # Bundle packages moved to the native importer (plugins/install/dsh.rs), which
        # owns their conformance corpus; this script must not keep a second implementation.
        package = self.fresh("dsh-package")
        package.mkdir()
        (package / "package.json").write_text(json.dumps({"dsh": {"bundle": {"patch": "./p.yml"}}}))
        self.refuse(self.args(bundle=package, dialect="dsh"), message="/plugin import dsh")
