...
export const docsWeb: DocsWebDict = {
  ...
  workflowLead:
    "Search recent threads or preview saved sessions from the history rail. Saved previews are read-only until you choose to resume. New threads have a searchable provider and model picker, and starter prompts fill the composer for review before you send. Finished fenced code blocks have a Copy button; other message text remains literal. The sidebar switches light and dark themes, and Latest returns to the newest message after you scroll back.",
  draftsBody:
    "Unsent drafts are kept per thread in this tab's session storage so they survive a reload. Storage is bounded and best effort; if unavailable or full, drafts remain in memory and cannot be guaranteed after a reload. Only the theme preference uses local storage. The client does not put Runtime tokens or configured provider credentials in either store.",
  shortcutsBody:
