...
export const docsModes: DocsModesDict = {
  ...
  modes: [
    ...
    ["Work", "Normal interactive coding. Codewhale can inspect, edit, and use tools; shell availability and approval prompts follow the active configuration and permission posture."],
  ...
  switchingLead:
    "When the composer is idle, press {tab} to cycle Plan → Work → Operate. When a completion menu is open, Tab accepts the completion; during an active turn, it can queue the current draft as the next follow-up.",
  switchingCommandLead: "Run /mode to open the picker, or switch directly:",
  permissionsTitle: "Permission postures",
  permissionsLead:
    "Plan is always Read Only. When the composer is idle in Work or Operate, press {shiftTab} to cycle Ask → Auto-Review → Full Access. Run {configCommand} to inspect or edit the current session permission; project or managed policy may lock or tighten it.",
  postures: [
    ["Ask", "Ask before tools that can make consequential changes."],
    ["Auto-Review", "Fully autonomous: it never stops to ask you. Proven-safe calls run, publish-like and destructive background actions are blocked, and anything else goes to a one-shot model review; high-risk calls and unresolved holds are denied rather than turned into a prompt."],
