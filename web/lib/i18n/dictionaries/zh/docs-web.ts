...
export const docsWeb: DocsWebDict = {
  ...
  workflowLead:
    "从历史侧栏搜索最近的对话或预览已保存的会话。预览保持只读，直到你选择恢复会话。创建对话时可以搜索提供商和模型；起始提示只会填入输入区，供你检查后发送。完整的围栏代码块带有复制按钮，其他消息文本按原文显示。侧栏可以切换明暗主题；向上翻阅后，点击 Latest 可回到最新消息。",
  draftsBody:
    "未发送的草稿按对话保存在当前标签页的会话存储中，刷新后可以恢复。存储有容量限制，采用尽力保存方式；存储不可用或已满时，草稿只保留在内存中，无法保证刷新后恢复。只有主题偏好使用本地存储。客户端不会把运行时令牌或已配置的提供商凭据写入这两种存储。",
  shortcutsBody:
