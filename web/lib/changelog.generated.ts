...
export const CHANGELOG: ChangelogRelease[] = [
  {
    ...
    "sections": [
      ...
      },
      {
        "heading": "Fixed",
        "items": [
          "Runtime API undo now restores the files of the turns it undoes, for every kind of thread. Before, a new thread's turns were not linked to their workspace snapshots, so patch-undo returned 201 and rewound the conversation but left the files changed, and file-revert refused. Each turn record now lists its snapshots in workspace_snapshots, the engine runs under the thread's own id across restarts, and a fork owns the turns it inherited. patch-undo restores only the files the…",
